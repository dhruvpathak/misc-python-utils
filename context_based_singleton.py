...
A shared state dict uses keys based on args,kwargs passed to the object to reference
unique state for that set of arguments. The kwargs dict is sorted at root level which
can ensure uniqueness in most practical cases
...
class Borg:
    ...
    def __init__(self,*args,**kwargs):
        context_key = hash('{0}{1}'.format(cPickle.dumps(args),cPickle.dumps(sorted(kwargs.items()))))
